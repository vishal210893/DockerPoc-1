...
1. **Build the Docker Image:**
   ...
   This command builds a Docker image named `dockerpoc-1` using the `Dockerfile` at `infra/docker/Dockerfile`.
...
2. **Install the Helm Chart:**
   Navigate to the `infra/helm/dockerpoc-app` directory and install the chart:

   ```bash
   cd infra/helm/dockerpoc-app
